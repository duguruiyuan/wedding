...
public class OrderController extends BaseController {
	...
	public ModelAndView saveU(PrintWriter out) throws Exception{
		...
		if(null == orderService.findByUId(pd)){
			if(Jurisdiction.buttonJurisdiction(menuUrl, "add")){orderService.saveU(pd);} //判断新增权限
	...
	public ModelAndView editU(PrintWriter out) throws Exception{
		...
		if(Jurisdiction.buttonJurisdiction(menuUrl, "edit")){orderService.editU(pd);}
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public Object hasU(){
		...
		try{
			pd = this.getPageData();
			if(orderService.findByUId(pd) != null){
				errInfo = "error";
			}
		} catch(Exception e){
	...
	public Object hasE(){
		...
		try{
			pd = this.getPageData();
			if(orderService.findByUE(pd) != null){
				errInfo = "error";
			}
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public Object hasN(){
		...
		try{
			pd = this.getPageData();
			if(orderService.findByUN(pd) != null){
				errInfo = "error";
			}
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public ModelAndView goEditU(){
		...
		try {
			...
			pd = orderService.findByUiId(pd);								//根据ID读取
	...
	public ModelAndView listUsers(Page page){
		...
		try{
			...
			page.setPd(pd);
			List<PageData>	orderList = orderService.listPdPageOrder(page);			//列出用户列表
	...
	public void deleteU(PrintWriter out){
		...
		try{
			pd = this.getPageData();
			if(Jurisdiction.buttonJurisdiction(menuUrl, "del")){orderService.deleteU(pd);}
			out.write("success");
			out.close();
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public Object deleteAllU() {
		...
		try {
			...
			if(null != USER_IDS && !"".equals(USER_IDS)){
				...
				if(Jurisdiction.buttonJurisdiction(menuUrl, "del")){orderService.deleteAllU(ArrayUSER_IDS);}
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		...
		try{
			if(Jurisdiction.buttonJurisdiction(menuUrl, "cha")){	
				...
				List<PageData> userList = orderService.listAllUser(pd);
