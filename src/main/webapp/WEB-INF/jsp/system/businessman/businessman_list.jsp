...
<body>
												...
												<tr>
													...
													<td style="width: 60px;" class="center">
														<div class='hidden-phone visible-desktop btn-group'>
															...
															<c:if test="${QX.edit == 1 }">
																<a title="编辑"
																	onclick="editProduct('${businessMan.id }');"
															...
															<c:choose>
																...
																<c:otherwise>
																	<c:if test="${QX.del == 1 }">
																		<a title="删除"
																			onclick="delOrder('${businessMan.id }','${businessMan.id }');"
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增商户";
			 diag.URL = '<%=basePath%>businessMan/goAddBusinessMan.do';
			 diag.Width = 450;
			 diag.Height = 555;
		...
		function editProduct(id){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑商户";
			 diag.URL = '<%=basePath%>businessMan/goEditBusinessMan.do?id='+id;
			 diag.Width = 350;
			 diag.Height = 555;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	<script type="text/javascript">
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>businessMan/deleteAllBusinessMans.do?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>businessMan/excel.do?USERNAME='+USERNAME+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&PRODUCTNAME='+PRODUCTNAME+'&STATUS='+STATUS;
		}
	</script>
</body>
</html>
