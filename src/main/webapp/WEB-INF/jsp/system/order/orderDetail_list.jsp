...
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="orderDetail/listOrderDetail.do" method="post"
						...
						<table border="0px">
							<tr>
								...
								<td style="vertical-align: top;"><select
									class="chzn-select" name="flow_id" id="flow_id"
									data-placeholder="状态" style="vertical-align: top; width: 79px;">
										<option value="">全部</option>
										<c:forEach items="${orderStates}" var="o">
											<option value="${o.name }"
												<c:if test="${o.name == pd.flow_id }">selected</c:if>>${o.name }</option>
										</c:forEach>
								</select></td>
												...
												<tr>
													...
													<td style="width: 60px;" class="center">
														<div class='hidden-phone visible-desktop btn-group'>
															...
															<c:if test="${QX.edit == 1 }">
																<a title="编辑" onclick="editOrderDetail('${order.id }');"
															...
															<c:choose>
																...
																<c:otherwise>
																	<c:if test="${QX.del == 1 }">
																		<a title="取消订单明细"
																			onclick="delOrderDetail('${order.id }','${order.id }');"
	...
	<script type="text/javascript">
		...
		function editOrderDetail(id){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑订单";
			 diag.URL = '<%=basePath%>orderDetail/goEditOrderDetail.do?ID='+id;
			 diag.Width = 350;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//删除
		function delOrderDetail(id,msg){
			bootbox.confirm("确定要取消订单["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>orderDetail/deleteOrderDetail.do?ID="+id+"&tm="+new Date().getTime();
	...
	<script type="text/javascript">
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>orderDetail/deleteAllOrderDetails.do?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>orderDetail/excel.do?USERNAME='+USERNAME+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&PRODUCTNAME='+PRODUCTNAME+'&STATUS='+STATUS;
		}
	</script>
</body>
</html>
